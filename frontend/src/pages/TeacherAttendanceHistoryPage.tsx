    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      {loading ? (
        <div className="min-h-[30vh] flex flex-col items-center justify-center p-8">
          <LoadingSpinner size="lg" label="Loading attendance sessions history..." />
        </div>
      ) : error ? (
        <Card className="p-8 text-center bg-white border-rose-200 space-y-4">
          <div className="w-12 h-12 rounded-2xl bg-rose-50 text-rose-600 mx-auto flex items-center justify-center">
            <XCircle className="w-6 h-6" />
          </div>
          <div>
            <h3 className="text-base font-bold text-slate-900 font-heading">Unable to load attendance sessions</h3>
            <p className="text-xs text-slate-500 mt-1">{error}</p>
          </div>
          <Button variant="outline" size="sm" onClick={fetchSessionsData} leftIcon={<RefreshCw className="w-3.5 h-3.5" />}>
            Try Again
          </Button>
        </Card>
      ) : sessions.length === 0 ? (
        ...
        <Card className="p-8 text-center bg-white border-slate-200 space-y-3">
          ...
          <p className="text-xs font-semibold text-slate-700">No sessions match your search or filter criteria.</p>
